...
1. 核对试点是否已有授权的 Responses 兼容模型地址/模型/凭据引用；若需要新增真实凭据、付费或改变权限，先请求 Owner 授权。当前已装配 HTTP 适配器，尚未实际启用和调用真实模型。
2. 继续实现“第二个”的已发送选项来源、机器人出站引用和人员定向；已新增自然归并模型路径，但不能用受控模型测试冒充多人真实群聊验收。

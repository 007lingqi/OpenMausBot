...
- 本批脚本/文档不改9089d71业务代码，复用其完整回归；76770为类型/语法/diff及工作区最终核对，终态需以回执确认后提交本批3文件。用户AGENTS.md/outputs不动。原账本a4115b快照：唯一Owner/租约各1，无运行任务/活动节点/待发Outbox，保留5个needs_configuration运行和1个collecting事项；切换前必须再次核对。

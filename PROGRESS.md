...
- 最新批次：schema 27 文档资源安全恢复已实现；13 文件 / 218 项相关回归通过。完整链 99898 在 db403f exit 0，272 文件通过 / 1 跳过、2833 项通过 / 18 跳过，类型检查、独立服务端编译和打包验证全通过。恢复只面向旧实例已知 ID，不能当作真实文档或主机重启验收。

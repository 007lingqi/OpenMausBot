...
- 总状态：进行中
- 当前恢复入口：读取 `SPEC.md`、`DECISIONS.md`、`PROGRESS.md`、`VERIFY.md`，然后继续第 4 阶段。
...
| 4. 完整回归与 Docker 真实验证 | 进行中 | 本地完整回归已通过，等待重建并替换试点容器 |
...
1. 运行最终完整回归、服务端类型检查和格式检查。
2. 创建本批次本地提交，不包含用户未跟踪的 `AGENTS.md`。
3. 重建并只替换试点 Docker 容器，验证健康、Stream 和执行模式。
4. 下一批接入真实钉钉文档/表格下载、解析和来源追踪；正文未读取时不得宣称已分析附件。

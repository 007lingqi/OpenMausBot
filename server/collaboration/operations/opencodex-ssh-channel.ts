...
export interface SshChannelCheckpoint {generation:string;attempts:number;}
export interface SshChannelCheckpointStore {
  read():SshChannelCheckpoint|null;
  write(value:SshChannelCheckpoint):void;
}
...
export function createPrivateSshChannel(operations:PrivateSshOperations,options:{checkpoint?:SshChannelCheckpointStore}={}){
  const restored=options.checkpoint?.read();
  let state:SshChannelState={status:'waiting',attempts:restored?.attempts??0},master:string|null=restored?.generation??null,storageFailed=false;
  ...
  const persist=(attempts:number)=>{
    try{options.checkpoint?.write({generation:master!,attempts});state.attempts=attempts;return true;}
    catch{storageFailed=true;state.status='failed';return false;}
  };
  async function reconcile():Promise<SshChannelState>{
    if(closing||storageFailed)return snapshot();
    ...
    if(attached){
      let healthy=false;try{healthy=await operations.inspect();}catch{/* Repair below consumes a durable attempt. */}
      if(healthy){state.status='connected';return snapshot();}
    }
    if(closing)return snapshot();
    // Reserve before any mutation: a process crash cannot refund an attempt.
    if(!persist(state.attempts+1))return snapshot();
    try{
      ...
      if(!persist(0)){attached=false;return snapshot();}
      state={status:'connected',attempts:0};
    }catch{
      attached=false;state.status=state.attempts>=3?'failed':'retrying';
    }
    return snapshot();
  }

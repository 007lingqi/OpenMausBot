...
describe("fenced document resource recovery", () => {
  ...
  it.each(["mismatch", "no_id", "malformed", "wrong_id", "wrong_name", "wrong_image"])("does not remove an unproven %s container", async fault => {
    const f = setup(fault);
    try {
      await f.recovery.run(f.current, 2000, () => {});
  ...
  it.each([true, false])("checks repository digest identity before recovery (matches=%s)", async valid => {
    ...
    try {
      ...
      expect(f.journal.readUnresolved()).toHaveLength(valid ? 1 : 2);

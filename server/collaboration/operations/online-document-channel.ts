...
export class PrivateSocketOnlineDocumentReader implements OnlineDocumentReader {
  private readonly authority: DwsOnlineDocumentReader;
  private readonly grants: readonly OnlineReadGrant[];
  private readonly socketPath: string;
  constructor(grants: readonly OnlineReadGrant[], socketPath: string) {
    validateOnlineDocumentSocketPath(socketPath);
  ...
  async read(input: OnlineReadSource, signal: AbortSignal): Promise<OnlineBodyReceipt> {
    ...
    // Probe mode constructs this adapter without inspecting or connecting to the socket.
    privateSocket(this.socketPath);
    const body = JSON.stringify({ version: 1, source });
    if (Buffer.byteLength(body) > INPUT_LIMIT) throw new Error("online_document_source_invalid");
    const raw = await new Promise<unknown>((resolveResponse, reject) => {
      let settled = false;
      ...
      const call = request({ socketPath: this.socketPath, path: ROUTE, method: "POST", agent: false, maxHeaderSize: 4096,
        headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(body) } }, response => {
        if (response.statusCode !== 200 || response.headers["content-type"] !== "application/json") { response.resume(); finish(unconfirmed()); return; }
      ...
      const timer = setTimeout(() => finish(unconfirmed()), 160000);
    ...
    const value = parsed.data, grant = this.grants.find(g => g.conversationId === source.conversationId && g.node === source.node)!;
    if (value.sourceEventId !== source.sourceEventId || value.normalizedHash !== source.normalizedHash || value.grantFingerprint !== fingerprint ||
      value.product !== grant.product || value.scope !== (grant.product === "doc" ? "full_document" : "all_worksheets") ||
...
export async function startOnlineDocumentGateway(options: { reader: OnlineDocumentReader; port?: number }): Promise<{ url: string; close(): Promise<void> }> {
  ...
  const server = createServer((incoming, response) => {
    ...
    const task = (async () => {
      ...
      try {
        ...
        const receipt = await options.reader.read(input.source, controller.signal);

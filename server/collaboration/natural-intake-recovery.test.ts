...
describe("Owner-bound natural requirement recovery", () => {
  ...
  it("upgrades v24 without inventing Owner authorization or releasing failed work", async () => {
    ...
    h.service.close();
    h.db.exec("DROP TABLE collaboration_natural_intake_recoveries; DROP TABLE collaboration_natural_intake_recovery_requests; DELETE FROM collaboration_schema_migrations WHERE version=25; PRAGMA user_version=24");
    h.db.close();
    const upgraded = openCollaborationLedger(join(h.directory, "collaboration"));
    expect(upgraded.migrationState).toEqual({ schemaVersion: 25, appliedMigrations: 25 }); upgraded.close();

...
describe("passive lifecycle recovery", () => {
  ...
  it("upgrades v20 without fabricating finalization evidence or losing process reservations", async () => {
    ...
    try {
      ...
      f.db.exec("DELETE FROM collaboration_schema_migrations WHERE version=21; PRAGMA user_version=20");
      expect(applyCollaborationMigrations(f.db)).toEqual({ schemaVersion: 21, appliedMigrations: 21 });

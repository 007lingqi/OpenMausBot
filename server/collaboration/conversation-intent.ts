...
export async function classifyConversationIntent(model: NaturalIntakeModelPort, input: ConversationIntentRequest,
  ...
  try {
    ...
    raw = await Promise.race([cancelled, model.complete({ signal, user, responseSchema: z.toJSONSchema(modelSchema), system: [
      ...
      "只限定当前想法先讨论、不发起新改动，用discussion_only。它只保留讨论，不创建/修改Spec，也不改变已有任务的运行状态；没有请求新建议时不要为了advice而编造多个方案。只有明确要求改变已有执行/候选/配置状态，才是control_request。不要将否定实施、暂不实施、讨论范围等同于暂停已有任务或请求Owner审批。意图本身不清时clarify。",
      "意图与事项归属是两个不同判断：已明确要查进度但未说明哪件事，仍是status_query，targetWorkItemId=null；不要改为clarify或猜一个事项。程序会就缺少的归属追问，并让下一句回答保持查询用途。这条规则同时适用于单意图和parts内分支；只有无法判断用户想查询、修改、讨论还是控制时才用clarify。",

...
export function readDiscussionImplementation(db: DatabaseSync, workItemId: string, sourceEventId: string): DiscussionImplementationContext | null {
  const raw = db.prepare("SELECT j.proposal_json,j.source_hash,j.context_outbox_sequence,e.normalized_json,e.principal_id,e.conversation_id " +
  ...
  if (receipt.sourceEventId !== sourceEventId || !receipt.quote.trim() || !event.text.includes(receipt.quote) ||

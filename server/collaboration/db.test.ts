...
const dropMappingRecovery = `DROP TABLE collaboration_coordinator_proofs; DROP VIEW collaboration_mapping_all_results; DROP VIEW collaboration_mapping_all_attempts;
...
describe("collaboration ledger", () => {
  it("upgrades v30 without inventing coordinator proofs or modifying existing rows", () => {
    ...
    db.exec("DROP TABLE collaboration_coordinator_proofs; DELETE FROM collaboration_schema_migrations WHERE version=31; PRAGMA user_version=30"); db.close();
    const upgraded = openCollaborationLedger(root); expect(upgraded.migrationState.schemaVersion).toBe(31); upgraded.close();
  ...
  it("upgrades v29 additively while retaining the three-attempt constraint and all mapping history", () => {
    ...
    const upgraded = openCollaborationLedger(directory); expect(upgraded.migrationState.schemaVersion).toBe(31); upgraded.close();
  ...
  it("upgrades v28 without inventing a previously authorized runtime policy", () => {
    ...
    expect(upgraded.migrationState).toEqual({ schemaVersion: 31, appliedMigrations: 31 }); upgraded.close();
    const after = new DatabaseSync(path);
    try {
  ...
  it("upgrades v26 retaining legacy resources without inventing instance ownership or recovery evidence", () => {
    ...
    before.exec(`DROP TABLE collaboration_coordinator_proofs; DROP VIEW collaboration_mapping_all_results; DROP VIEW collaboration_mapping_all_attempts; DROP TABLE collaboration_mapping_recovery_results; DROP TABLE collaboration_mapping_recovery_attempts; DROP TABLE collaboration_verification_runtime_policies; DROP TABLE collaboration_delivery_queries; DROP TRIGGER document_resources_recovery_immutable;
  ...
  it("upgrades v21 without fabricating historical attachment failures", () => {
    ...
    db.close();
    const upgraded = openCollaborationLedger(directory);
    expect(upgraded.migrationState).toEqual({ schemaVersion: 31, appliedMigrations: 31 });
    upgraded.close();
  ...
  it("upgrades v19 without losing existing reservations and creates immutable execution lifecycle tables", () => {
    ...
    const upgraded = openCollaborationLedger(directory);
    expect(upgraded.migrationState.schemaVersion).toBe(31); upgraded.close();
    const after = new DatabaseSync(join(directory, COLLABORATION_DATABASE_NAME));
    try {
  ...
  it("reopens without duplicating or recreating schema state", () => {
    ...
    expect(second.migrationState).toEqual({ schemaVersion: 31, appliedMigrations: 31 });
    second.close();

    const after = new DatabaseSync(join(directory, COLLABORATION_DATABASE_NAME));
    expect(after.prepare("SELECT count(*) AS count FROM collaboration_schema_migrations").get()).toEqual({ count: 31 });

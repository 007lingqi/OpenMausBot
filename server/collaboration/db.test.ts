...
describe("collaboration ledger", () => {
  ...
  it("reopens without duplicating or recreating schema state", () => {
    ...
    expect(second.migrationState).toEqual({ schemaVersion: 17, appliedMigrations: 17 });
    second.close();

    const after = new DatabaseSync(join(directory, COLLABORATION_DATABASE_NAME));
    expect(after.prepare("SELECT count(*) AS count FROM collaboration_schema_migrations").get()).toEqual({ count: 17 });

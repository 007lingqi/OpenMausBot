...
export class InboundMessageProcessor {
  ...
  processDingTalkMessage(message: DingTalkInboundMessage): InboundMessageOutcome {
    ...
    try {
      ...
      if (this.database.prepare("SELECT 1 FROM collaboration_owner_text_commands WHERE source_event_id=? AND json_extract(outcome_json,'$.kind')='token_action'").get(sourceEventId)) {
        throw new Error("token_action_event_conflict");
      }
      if (this.database.prepare("SELECT 1 FROM collaboration_owner_text_commands WHERE source_event_id=? AND json_extract(outcome_json,'$.kind')='owner_query'").get(sourceEventId)) {
        throw new Error("owner_query_event_conflict");
      }

...
最新装配：headless 现已支持明确的 `OMB_ACCEPTANCE_MAPPING_*` 配置，详见 `../../../packaging/collaboration/docker/README.models.md`。默认仍关闭；模型/端点/凭据引用/规则版本变化会自动改变有效策略身份。最终完成判定会重读映射持久收据并核对两套测试契约。上段“尚未装配”是历史状态，不能据此绕开新增配置约束；真实授权配置及语义效果尚未验收。

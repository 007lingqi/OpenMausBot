...
try {
  await build({ entryPoints: { probe: join(root, 'server/collaboration/operations/online-document-docker.smoke-probe.ts'),
    'online-document-relay': join(root, 'server/collaboration/operations/online-document-relay.ts') }, outdir: staging,
  ...
  id = docker(['create', '--pull', 'never', '--name', name, '--label', 'omb.fixture=document-relay', '--network', 'none', '--read-only',
    ...
    '--mount', `type=bind,source=${staging},target=/fixture,readonly`, '--user', '0:0', '--entrypoint', '/usr/local/bin/node', image, '/fixture/probe.mjs']).trim();
  ...
  console.log(JSON.stringify({ result: 'passed', context, container: name, id, fixtureOnly: true,
    rootDirectSocket: 'EACCES', relayUid: 501, relayGid: 1000, realDwsReads: 0 }));
} finally {

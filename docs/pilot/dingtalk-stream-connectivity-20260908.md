...
因此6fd5f0恢复已验证的兼容修复版，当前d5252db1…/d55df162… running/healthy、重启0、unless-stopped，systemd active/enabled；当前实例实际Stream connected、ready/execute，观察139秒一次连接、0断开/错误。当前数据未被备份覆盖，schema37及两代私有备份保留。所有会话终态；无定时重试，Goal保持宿主paused。下一步仍是执行环境可用后的最终候选安全切换，而非再向用户索要同样业务许可。结构化证据与精确映射见evidence/conversation-stream-recovery-052b42c.json。
